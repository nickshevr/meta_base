...
## 13. Post-MVP Technical Roadmap

- MVP стартует на JSON для скорости реализации.
- Следующий этап после подтверждения ценности: миграция storage backend на SQLite без изменения доменной логики (через repository abstraction).
- Целевая эксплуатация: Yandex Cloud.
- IaC-подход: Terraform как единый способ подъема/изменения инфраструктуры.

### Infrastructure direction (post-MVP)
1. Terraform-манифесты для `dev` и `prod` окружений.
2. Хостинг приложения/worker в Yandex Cloud (вариант определяется на этапе infra design).
3. Секреты (LLM API keys) через управляемое хранение секретов.
4. Бэкапы SQLite и отчетов, плюс процесс восстановления.
